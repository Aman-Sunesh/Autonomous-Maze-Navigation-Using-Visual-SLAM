...
class VisualStuckDetector:
    ...
    def filter_actions(self, curr_img, commanded_v, commanded_w, dt, is_near_wall):
        ...
        # # If dx violently flips sign (dx * prev_dx < 0) and the jump is suspiciously large, 
        # # it caught the wrong repeating texture. Ignore the flip and keep the momentum.
        # if self.prev_dx != 0.0 and (dx * self.prev_dx < 0) and abs(dx - self.prev_dx) > 10:
        #     print("fuck")
        #     dx = self.prev_dx
        
        # # Save the true momentum for the next frame
...
def run_slam():
    ...
    odom = Odometry(initial_x=5.2, initial_y=-5.2, initial_theta=np.pi/2)    # spawn robot in the middle of the hallway
