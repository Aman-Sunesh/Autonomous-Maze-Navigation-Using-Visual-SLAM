...
class KeyboardPlayerPyGame(Player):
                ...
                pickle.dump(
                    {
                        ...
                        "goal_world_coords": self.goal_world_coords,
                    },
                    f,
                )
    ...
    def get_autonomous_action(self):
        ...
        while len(self.global_path) > 1:
            ...
            if np.linalg.norm(wp0 - robot) < 0.16:
    ...
    def _setup_goal(self):
        ...
        if os.path.exists(PATH_CACHE_FILE):
            try:
                ...
                if cached.get("goal_world_coords") is not None:
                    self.goal_world_coords = tuple(cached["goal_world_coords"])
                    print(f"[GOAL] Loaded cached goal: ({self.goal_world_coords[0]:.2f}, {self.goal_world_coords[1]:.2f})")
                    return

            except Exception as e:
                print(f"[GOAL] Goal cache load failed: {e}")
        ...
        if goal_file in self.frame_poses:
            ...
            # Compute the essential matrix to refine the target position
            ...
            if match_img is not None:
                ...
                if len(good) >= 8: 
                    ...
                    if E is not None and E.shape == (3, 3):
                        ...
                        # Since E matrix doesn't give the scale, recover the scale
                        ...
                        if inlier_pts_target.shape[1] > 0:
                            # Triangulate the 3D points using the unit scale (Z_unit)
                            P1 = self.K @ np.hstack((np.eye(3), np.zeros((3,1))))
                            P2 = self.K @ np.hstack((R, t))
                            points_4d = cv2.triangulatePoints(P1, P2, inlier_pts_target, inlier_pts_match)
                            points_3d = points_4d[:3, :] / points_4d[3, :]
                            
                            # Build the Sky Mask to find true wall depths
                            h, w = front.shape[:2]
                            ...
                            # Compare Triangulated Depth vs True Depth
                            ...
                            # Take the Median to ignore noise
