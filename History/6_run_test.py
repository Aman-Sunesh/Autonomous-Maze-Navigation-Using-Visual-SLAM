...
class KeyboardPlayerPyGame(Player):
    ...
    def __init__(self, n_clusters: int = 128, subsample_rate: int = 5, top_k_shortcuts: int = 30):
        ...
        if os.path.exists("cache/slam_map_walls.png") and os.path.exists("cache/frame_poses.json"):
            self.slam_map = cv2.imread("cache/slam_map_walls.png")
    ...
    def see(self, fpv):
        ...
        if self._state and self._state[1] == Phase.NAVIGATION:
            ...
            # --- 2. KINEMATIC INTEGRATION (Runs every frame) ---
                ...
                # Apply kinematics directly (No map collision checks)
                self.odom.x += v * dt * np.cos(self.odom.theta)
                self.odom.y += v * dt * np.sin(self.odom.theta)
