...
OFFSET_M = 0.2              # start robot at the center of the grid
...
MATCH_THRESH = 0.15         # increase: larger area to snap position fix
...
class WallDetector:
    ...
    """
    def __init__(self, K, camera_height=CAMERA_HEIGHT, wall_height=WALL_HEIGHT, max_depth=MAX_DEPTH): 
        """
...
class MapBuilder:
    ...
    def __init__(self, map_size=800, scale=60.0):
        ...
        # Create a blank gray canvas with map_size by map_size pixels
        self.grid = np.ones((self.map_size, self.map_size, 3), dtype=np.uint8) * 127
        
        grid_m = GRID_M        # set grid size as 0.4m x 0.4m
        offset_m = OFFSET_M    # start robot at the center of the grid

        # Conver grid in meters to pixels
        grid_px = int(grid_m * self.scale)
        offset_px = int(offset_m * self.scale)

        # Draw the background grid with grid_m spacing
        grid_color = (100, 100, 100)
        for x in range(self.offset_x + offset_px, self.map_size, grid_px):  # center to right edges
            cv2.line(self.grid, (x, 0), (x, self.map_size), grid_color, 1)
        for x in range(self.offset_x - offset_px, -1, -grid_px):            # center to left edges
            cv2.line(self.grid, (x, 0), (x, self.map_size), grid_color, 1)
        for y in range(self.offset_y + offset_px, self.map_size, grid_px):  # center to bottom edges
            cv2.line(self.grid, (0, y), (self.map_size, y), grid_color, 1)
        for y in range(self.offset_y - offset_px, -1, -grid_px):            # center to top edges
            cv2.line(self.grid, (0, y), (self.map_size, y), grid_color, 1)
...
class ManhattanConstraint:
    ...
    def __init__(self, step_size=STEP_SIZE, num_bins=NUM_BINS, angle_thresh=ANGLE_THRESH,
                 ...
                 grid_m=GRID_M, offset_m=OFFSET_M, match_thresh=MATCH_THRESH, viz=False):
        """
        ...
        if len(local_walls) > 15:   # set threshold of 15 for robustness
            ...
            if np.sum(inlier_mask_segments) > 5:    # set threshold of 15 for robustness
                ...
                if len(valid_local_pts) > 0:
                    ...
                    if is_horizontal:
                        # Snap the wall to the nearest 0.4m grid line
                        target_y = np.round((mean_wall_y - self.offset_m) / self.grid_m) * self.grid_m + self.offset_m
                        ...
                        # Snap the wall to the nearest 0.4m grid line
                        target_x = np.round((mean_wall_x - self.offset_m) / self.grid_m) * self.grid_m + self.offset_m
    ...
    def snap_wall_points(self, global_wall_points):
        ...
        for wx, wy in global_wall_points:           
            ...
            snapped_x = round((wx - self.offset_m) / self.grid_m) * self.grid_m + self.offset_m
            snapped_y = round((wy - self.offset_m) / self.grid_m) * self.grid_m + self.offset_m
...
def run_slam():
    ...
    odom = Odometry(initial_x=0.0, initial_y=0.0, initial_theta=np.pi/2)    
