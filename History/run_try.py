...
GOAL_REFINEMENT_CACHE_FILE = os.path.join(CACHE_DIR, "goal_refinement_cache.pkl")
...
class KeyboardPlayerPyGame(Player):
    ...
    def _quantize_cache_pixel(self, value, bucket=8):
        """Quantize a pixel coordinate so nearby replans reuse the same cached A* path."""
        return int(round(value / float(bucket)) * bucket)

    def _load_pickle_cache(self, path):
        """Best-effort pickle cache load."""
        ...
        try:
            ...
            print(f"[CACHE] Failed loading {path}: {e}")
            return {}

    def _save_pickle_cache(self, path, data):
        """Best-effort pickle cache save."""
        try:
            with open(path, "wb") as f:
                pickle.dump(data, f)
        except Exception as e:
            print(f"[CACHE] Failed saving {path}: {e}")
    ...
    def plan_astar_path(self):
        ...
        cache_key = (
            "turn_penalty_v3",
            TURN_COST,
            self._quantize_cache_pixel(start_px),
            self._quantize_cache_pixel(start_py),
            self._quantize_cache_pixel(goal_px),
            self._quantize_cache_pixel(goal_py),
        )
        cached = self._load_pickle_cache(PATH_CACHE_FILE)
        ...
        # ---------------------------------------------------------------
        self._save_pickle_cache(PATH_CACHE_FILE, {"key": cache_key, "path": self.global_path})
    ...
    def see(self, fpv):
        ...
        if self._state and self._state[1] == Phase.NAVIGATION:
            ...
            if self.viz and self.odom is not None:
                self.viz_counter += 1
                if self.viz_counter % self.viz_every_n == 0:
    ...
    def _setup_goal(self):
        ...
        # Cache the expensive pose-refinement result for repeated runs with the same target.
        front_hash = hashlib.md5(front.tobytes()).hexdigest()
        ...
        if goal_file in self.frame_poses:
            ...
            goal_cache_key = ("goal_refine_v1", goal_file, front_hash)
            goal_cache = self._load_pickle_cache(GOAL_REFINEMENT_CACHE_FILE)
            cached_goal = goal_cache.get(goal_cache_key)
            if cached_goal is not None:
                self.goal_world_coords = tuple(cached_goal)
                print(
                    f"[GOAL] Loaded cached refined goal: "
                    f"({self.goal_world_coords[0]:.2f}, {self.goal_world_coords[1]:.2f})"
                )
                return
            ...
            if match_img is not None:
                ...
                if len(good) >= 8: 
                    ...
                    if E is not None and E.shape == (3, 3):
                        ...
                        goal_cache[goal_cache_key] = self.goal_world_coords
                        self._save_pickle_cache(GOAL_REFINEMENT_CACHE_FILE, goal_cache)
